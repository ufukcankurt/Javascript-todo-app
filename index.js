...
function load() {
  ...
  if (Array.isArray(_endtodos)) {
    _endtodos.forEach((value, key) => {
      createContentEnd(value["value"], value["id"]);
    });
  } else {
    _endtodos = [];
  }
...
function createContent(newTodoValue, uuid = null) {
...
// todo:: create frontend content and add event listener for button | This for EndTodos 
function createContentEnd(newTodoValue, uuid = null) {
  const newTodoItem = document.createElement("div");
  newTodoItem.classList.add("todo");
